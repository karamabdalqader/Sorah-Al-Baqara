...
<div class="flex items-center gap-3 bg-white dark:bg-[#1e1e1e] border border-accent/30 dark:border-accent/20 shadow-xl px-6 py-3 rounded-full transition-colors duration-300" id="save-indicator">
    <span class="material-symbols-outlined text-accent text-[22px] fill-current">bookmark</span>
    <span class="text-[11px] uppercase tracking-[0.15em] font-bold font-ui text-text-ui dark:text-gray-300 transition-colors">تم حفظ التقدم</span>
</div>
...
<script>
    ...
    function updateProgressText() {
        ...
        segments.forEach(segment => {
            ...
            } else if (rect.top <= viewportMiddle && rect.bottom > 0) {
            	currentPage = segment.dataset.page;
            }
        });
    ...
    function showSaveIndicator(customText) {
        if (customText) {
            saveIndicator.children[1].textContent = customText;
        } else {
            saveIndicator.children[1].textContent = "تم حفظ التقدم";
        }
